...
class AssetPathResolver {
  ...
  private cdnPaths: string[] = [];
  
  addFallbackPath(path: string) {
    this.fallbackPaths.push(path.endsWith('/') ? path : path + '/');
  }
  
  addCDNPath(path: string) {
    this.cdnPaths.push(path.endsWith('/') ? path : path + '/');
  }
  
  async resolveAsset(relativePath: string): Promise<Response> {
    const paths = [
...
self.addEventListener('fetch', (event) => {
  ...
  if (url.pathname.match(/\.(js|css)$/)) {
    event.respondWith(
      pathResolver.resolveAsset(url.pathname)
