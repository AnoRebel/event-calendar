...
export function useDragAndDropSystem(config: Partial<DragDropConfig> = {}) {
  ...
  const createDraggableEvent = (event: CalendarEvent) => {
    ...
    // vue-dnd-kit v2: caller owns the ref, `data` is a factory, and a payload
    // factory returns [index, sourceItems]. The event id must be stable so the
    // draggable survives remounts inside virtualized views.
  ...
  const createDroppableZone = (config: ReturnType<typeof createDragConfig>) => {
    ...
    // vue-dnd-kit v2: caller owns the ref; there is no `id`/`disabled`/`onDrop`
    // at the top level — the zone identity goes in `data`, and drop handling is
    // nested under `events`. `isDragOver` is an IPlacement (or undefined), so we
    // expose a boolean `isOver` to keep this composable's consumer API stable.
    const elementRef = ref<HTMLElement | null>(null)
      ...
      events: {
        onDrop: (event: IDragEvent) => {
          try {
            ...
            const first = event.draggedItems[0]
            const droppedEvent = (first?.item ?? first?.data) as CalendarEvent | undefined
    ...
    const isOver = computed(() => isDragOver.value !== undefined)
