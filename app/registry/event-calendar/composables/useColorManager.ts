...
export function useColorManager(
  ...
  // Track which colors are currently in use — both colors already on events and
  // colors handed out by assignUniqueColor (recorded in colorAssignments) so that
  // successive assignments before events are persisted don't collide.
  const usedColors = computed(() => {
    ...
    colorState.value.colorAssignments.forEach(color => {
      used.add(color)
    })
    return used
  })
