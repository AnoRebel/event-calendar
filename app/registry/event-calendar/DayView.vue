...
// Single shared drag-and-drop system instance.
...
// Function to get DnD config - reads live props so it always reflects the
// current day, even though the droppable zones are registered once in setup.
...
// vue-dnd-kit v2 registers droppables via inject + lifecycle hooks, so the
// zones MUST be created during setup — not in onMounted/watch. The element refs
// are stable; drop logic recomputes config from live props at drop time.
const allDayZone = createDroppableZone(getDndConfigForDayZone("allDay"))
const timedZone = createDroppableZone(getDndConfigForDayZone("timed"))
